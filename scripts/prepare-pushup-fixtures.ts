...
import { buildPushupFixtureCorpus } from '../lib/debug/pushup-fixture-corpus';

const ROOT = process.cwd();
const FIXTURE_DIR = join(ROOT, 'tests', 'fixtures', 'pushup-tracking');
...
function main(): void {
  ...
  const corpus = buildPushupFixtureCorpus();
  ...
  console.log(`[fixtures:pushup:prepare] dir=${FIXTURE_DIR}`);
  for (const entry of written) {
    const state = entry.changed ? 'updated' : 'ok';
    console.log(`${state} ${entry.fileName} ${entry.checksum}`);
  }
  console.log(`[fixtures:pushup:prepare] total=${written.length} changed=${changedCount}`);
}

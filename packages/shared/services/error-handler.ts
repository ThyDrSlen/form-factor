 ...
 *
 * Unlike the Expo version, this does NOT depend on react-native Platform
 * or expo-constants. Platform metadata can be passed in via ErrorContext.
 */

import { errorWithTs } from '../utils/logger';
...
export function logError(
  err: AppError,
  ctx?: ErrorContext,
  platformInfo?: { platform?: string; appVersion?: string }
): void {
  ...
  errorWithTs('[Error]', {
    ...
    appVersion: platformInfo?.appVersion ?? 'unknown',
  });
}

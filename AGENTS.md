...
Never commit `.env` files or Supabase keys—load them via Expo secrets or CI. `eas.json` defines `development`, `preview`, `staging`, and `production` profiles, so confirm the target before running `eas build`. Device overrides live in `app.config.ts`, and certificate changes should reference `docs/BUILD_CONFIGURATION.md` plus `lifeguard.yaml`.
